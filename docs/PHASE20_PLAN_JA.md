...
| 7 | `crypto/transport.js`のUI接続 | 送受信の抽象化導線 | ⏳ 未着手 |
| 8 | 互換データ移行実装 | 既存ユーザーデータ移行ロジック | ⏳ 未着手 |
| 9 | BLE送信の信頼性向上 | ACK/再送/タイムアウト | ⏳ 未着手 |
| 10 | BLE受信の堅牢化 | 欠落・重複・順不同処理 | ⏳ 未着手 |
| 11 | Outbox/Inbox運用接続 | オフラインキュー送達 | ⏳ 未着手 |
| 12 | 主要統合テスト追加 | 暗号化→送信→復号の自動検証 | ⏳ 未着手 |
| 13 | E2E最小セット導入 | 主要ユーザーフローE2E | ⏳ 未着手 |
| 14 | CI品質ゲート強化 | lint/typecheck/test分離ゲート | ⏳ 未着手 |
| 15 | セキュリティ監査準備 | 監査前チェック項目の実施 | ⏳ 未着手 |
| 16 | Group Messaging最小実装 | グループ作成・送受信・復号 | ⏳ 未着手 |
| 17 | TypeScript段階導入 | 重要境界の型安全化 | ⏳ 未着手 |
| 18 | パフォーマンス改善 | Web Worker・DB最適化 | ⏳ 未着手 |
| 19 | 運用文書最終化 | Runbook/FAQ/制約明記 | ⏳ 未着手 |
| 20 | リリース判定 | 完成判定チェック全通過 | ⏳ 未着手 |
